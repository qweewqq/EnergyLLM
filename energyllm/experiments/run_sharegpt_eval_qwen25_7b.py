...
PPO_V5_DIR = os.path.join(PROPOSED_DIR, "ablation_qwen25_7b/EnergyLLM")
DYNAMOLLM_DIR = os.path.join(BASE_DIR, "dynamollm")

SHAREGPT_DATASET = os.path.join(PROPOSED_DIR, "prompt_datasets_jsonl/sharegpt_en_mixed_1000.jsonl")
...
OUTPUT_DIR = os.path.join(BASE_DIR, "generalization/sharegpt_results_qwen25_7b")
...
def check_dataset():
    if not os.path.exists(SHAREGPT_DATASET):
        print(f"Dataset not found: {SHAREGPT_DATASET}")
        return False
    
    with open(SHAREGPT_DATASET, 'r') as f:
        count = sum(1 for _ in f)
    print(f"ShareGPT dataset: {count} entries")
    return True
...
def run_all_tests():
    ...
    results_file = os.path.join(OUTPUT_DIR, f"sharegpt_results_{timestamp}.json")
...
def generate_summary(results, output_dir, timestamp):
    ...
    csv_file = os.path.join(output_dir, f"sharegpt_summary_{timestamp}.csv")
    df.to_csv(csv_file, index=False)
    print(f"Summary table saved to: {csv_file}")
    
    print("\n" + "="*80)
    print("ShareGPT Dataset Baseline Comparison Results (Qwen2.5-7B)")
    print("="*80)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ShareGPT Baseline Comparison - Qwen2.5-7B')
    parser.add_argument('--method', type=str, default=None, 
                        help='Specify test method (Static-High, DVFS-Only, Batch-Only, Reactive-DVFS, Token-Aware, DynamoLLM-DVFS, PPO-v5)')
    parser.add_argument('--slo', type=float, default=None, help='Specify SLO (e.g. 11.0)')
    parser.add_argument('--rate', type=float, default=None, help='Specify arrival rate (e.g. 0.8)')
    parser.add_argument('--runs', type=int, default=10, help='Number of runs per config (default 10)')
    parser.add_argument('--duration', type=int, default=60, help='Duration in seconds per run (default 60)')
    ...
    print("="*60)
    print("ShareGPT Baseline Comparison - Qwen2.5-7B")
    print("="*60)
