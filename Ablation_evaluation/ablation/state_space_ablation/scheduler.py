"""
消融实验调度器: w/o SLO & Latency Features
去掉 recent_slo 和 recent_latency，状态空间从 7 维减少到 5 维
"""
...
class RLScheduler:
    """消融实验调度器: w/o SLO & Latency Features (5维状态)"""
    ...
    def __init__(self, model_path, slo_latency_sec, dataset_path=None, arrival_rate=0.8):
        ...
        print(f"消融调度器 (w/o SLO & Latency Features, 5维状态): SLO={slo_latency_sec}s")
    ...
    def _get_state(self, current_time):
        """
        消融: 5维状态空间（去掉 recent_slo, recent_latency）
        [0] queue_size, [1] freq_idx, [2] arrival_rate, [3] avg_wait, [4] slo_target
        """
        ...
        state = np.array([
            ...
            # 消融: 去掉 recent_slo, recent_latency
    ...
    def _print_stats(self):
        ...
        print("\n" + "=" * 50)
        print(f"SLO 满足率: {satisfied/total*100:.2f}%")
        print(f"平均延迟: {df['e2e_latency'].mean():.2f}s")
        print(f"平均能耗: {energy/total:.2f} J/req")
