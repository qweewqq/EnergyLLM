...
from config import (
    ...
    TRAIN_CONFIG, PPO_CONFIG, EXPERIMENT_CONFIGS, FIXED_BATCH
)
...
def evaluate(agent, n_episodes=3):
    ...
    for slo, rate in EXPERIMENT_CONFIGS:
        ...
        results[key] = {'slo_rate': total_slo / n_episodes, 'energy': total_energy / n_episodes}
    return results


def train(agent, total_timesteps, eval_freq=10000, save_freq=100000):
    print("\n" + "=" * 50)
    print(f"消融实验: Fixed Batch ({FIXED_BATCH})")
    print("=" * 50)
        ...
        for _ in range(n_steps):
            ...
            if done:
                ...
                if episode % 50 == 0:
                    ...
                    print(f"Ep {episode}, Step {timestep}: SLO%={slo:.1f}%, Energy={energy:.1f}")
                episode_reward = 0
                state = env.reset()
...
def main():
    ...
    agent = PPOAgent()
    train(agent, args.timesteps, eval_freq=TRAIN_CONFIG['eval_freq'], save_freq=TRAIN_CONFIG['save_freq'])
