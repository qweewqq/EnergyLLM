...
class RLScheduler:
    ...
    def __init__(self, model_path, slo_latency_sec, dataset_path=None, arrival_rate=0.8):
        ...
        print(f"消融调度器 (w/o Energy Reward): SLO={slo_latency_sec}s")
    ...
    def _get_state(self, current_time):
        ...
        if queue_size > 0:
            ...
            avg_input = np.mean([r['input_len'] for r in queue_list])
            avg_output = np.mean([r['output_len'] for r in queue_list])
