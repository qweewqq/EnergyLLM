"""
消融实验: w/o Energy Reward
环境与 PPO-v4 完全相同，只是奖励函数中去掉了能耗项
"""
...
class LLMSchedulingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 奖励计算（与 PPO-v4 相同，但 config 中 energy_weight=0, low_energy_bonus=0）
        reward = 0.0
        for task in tasks:
        ...
        # 能耗（使用 config 中的值，消融时 energy_weight=0, low_energy_bonus=0）
        energy = power * latency
        self.total_energy += energy
        energy_per_req = energy / actual_batch
        reward -= REWARD_CONFIG['energy_weight'] * energy_per_req  # 消融: 0.0 * energy = 0
        
        if energy_per_req < REWARD_CONFIG.get('energy_threshold', 170):
            reward += REWARD_CONFIG.get('low_energy_bonus', 0.0)  # 消融: 0.0
