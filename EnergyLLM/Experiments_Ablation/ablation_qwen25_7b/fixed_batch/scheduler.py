"""
消融实验调度器: Fixed Batch (16) - Qwen2.5-7B版本
固定批次大小，只调整频率
"""
...
class RLScheduler:
    """消融实验调度器: Fixed Batch (16) - Qwen2.5-7B"""
    ...
    def __init__(self, model_path, slo_latency_sec, dataset_path=None, arrival_rate=0.8):
        ...
        print(f"[Qwen2.5-7B] 消融调度器 (Fixed Batch {FIXED_BATCH}): SLO={slo_latency_sec}s")
    ...
    def _predict(self, freq, batch_size, avg_input_len=150, avg_output_len=100):
        ...
        features = {'gpu_frequency_mhz': freq, 'batch_size': batch_size,
                    'avg_input_len': avg_input_len, 'avg_output_len': avg_output_len}
        X = pd.DataFrame([features])[self.feature_names]
        return max(0.1, self.latency_model.predict(X)[0]), max(50, self.power_model.predict(X)[0])
    
    def _get_state(self, current_time):
        queue_size = len(self.request_queue)
        avg_wait = np.mean([current_time - r['arrival_time'] for r in self.request_queue]) if queue_size > 0 else 0
        ...
        return np.array([min(queue_size / MAX_QUEUE_SIZE, 1.0), self.current_freq_idx / (len(FREQ_OPTIONS) - 1),
            recent_slo_rate, min(recent_lat / 30, 1.0), min(self.arrival_rate / 2.0, 1.0),
            min(avg_wait / 30, 1.0), (self.slo_latency_sec - 8) / 8], dtype=np.float32)
    
    def _action_to_config(self, action):
        freq = FREQ_OPTIONS[action]
        return freq, FIXED_BATCH, action
    ...
    def _request_generator(self, arrival_rate):
        ...
        while not self.stop_event.is_set():
            ...
            self.request_queue.append({'id': request_id, 'arrival_time': time.time(),
                'input_len': item.get('input_len', 150), 'output_len': item.get('output_len', 100)})
            request_id += 1

    def run_simulation(self, duration_sec, arrival_rate):
        self.arrival_rate = arrival_rate
        print(f"\n开始模拟: {duration_sec}s, {arrival_rate} req/s, SLO={self.slo_latency_sec}s")
        ...
        try:
            while time.time() - start_time < duration_sec:
                ...
                switch_overhead = self._set_gpu_frequency(freq)
                ...
                for task in tasks:
                    ...
                    self.history.append({'timestamp': end_time, 'freq': freq, 'batch_size': batch_to_process,
                        'latency': latency, 'power': power, 'e2e_latency': e2e, 'slo_satisfied': satisfied})
        finally:
    ...
    def _print_stats(self):
        ...
        total, satisfied = len(df), df['slo_satisfied'].sum()
        energy = (df['power'] * df['latency']).sum()
        print(f"\nSLO 满足率: {satisfied/total*100:.2f}%, 平均延迟: {df['e2e_latency'].mean():.2f}s, 平均能耗: {energy/total:.2f} J/req")
