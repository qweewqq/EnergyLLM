"""
消融实验: Fixed Batch (16) - Qwen2.5-7B版本
固定批次大小，只调整频率
"""
...
from config import (
    ...
    EXPERIMENT_CONFIGS, FIXED_BATCH
)


class LLMSchedulingEnv(gym.Env):
    """消融: Fixed Batch (16) - Qwen2.5-7B版本"""
    ...
    def __init__(self, slo_target=DEFAULT_SLO, arrival_rate=0.8, randomize_config=False):
        ...
        # 消融: 动作空间只有频率选择
        self.action_space = spaces.Discrete(ACTION_DIM)  # 15
    ...
    def _load_lut(self):
        ...
        try:
            ...
            print(f"[Qwen2.5-7B] LUT: {len(self.lut)} 条")
