"""
消融实验调度器: Fixed Freq (1200MHz) - Qwen2.5-7B版本
固定频率，只调整批次大小
"""
...
class RLScheduler:
    """消融实验调度器: Fixed Freq (1200MHz) - Qwen2.5-7B"""
    ...
    def __init__(self, model_path, slo_latency_sec, dataset_path=None, arrival_rate=0.8):
        ...
        print(f"[Qwen2.5-7B] 消融调度器 (Fixed Freq {FIXED_FREQ}MHz): SLO={slo_latency_sec}s")
    ...
    def _get_state(self, current_time):
        ...
        return np.array([min(queue_size / MAX_QUEUE_SIZE, 1.0), FIXED_FREQ_IDX / (len(FREQ_OPTIONS) - 1),
    ...
    def _action_to_config(self, action):
        return FIXED_FREQ, BATCH_OPTIONS[action], FIXED_FREQ_IDX
