...
ABLATION_DIR = os.path.join(PROPOSED_DIR, "ablation_qwen25_7b")
...
OUTPUT_DIR = os.path.join(BASE_DIR, "generalization/wildchat_ablation_results_qwen25_7b")
...
ABLATION_VARIANTS = {
    ...
    'wo_energy_reward': {
        ...
        'type': 'rl',
        'dir': os.path.join(ABLATION_DIR, 'wo_energy_reward'),
        'model': 'checkpoints/ppo_final.pt'
    },
    'fixed_freq': {
        'name': 'Fixed Freq (1200MHz)',
        'type': 'rl',
        'dir': os.path.join(ABLATION_DIR, 'fixed_freq'),
        'model': 'checkpoints/ppo_final.pt'
    },
    'fixed_batch': {
        'name': 'Fixed Batch (16)',
        'type': 'rl',
        'dir': os.path.join(ABLATION_DIR, 'fixed_batch'),
        'model': 'checkpoints/ppo_final.pt'
    },
    'full': {
        'name': 'EnergyLLM (Ours)',
        'type': 'rl',
        'dir': os.path.join(ABLATION_DIR, 'wo_token_aware'),
        'model': 'checkpoints/ppo_final.pt'
    }
}
...
def check_models():
    ...
    for key, variant in ABLATION_VARIANTS.items():
        if variant['type'] == 'heuristic':
            exists = os.path.exists(LATENCY_MODEL_PATH)
            status = "✓" if exists else "✗"
            print(f"  {status} {variant['name']}: Using Qwen2.5-7B ML model")
        else:
...
def run_all_experiments():
    print("=" * 70)
    print("WildChat Ablation Experiments - Qwen2.5-7B")
    print("=" * 70)
...
def save_results(all_results, timestamp):
    ...
    print("\n" + "=" * 80)
    print("Average Metrics by Variant (Qwen2.5-7B)")
    print("=" * 80)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='WildChat Ablation Experiments - Qwen2.5-7B')
